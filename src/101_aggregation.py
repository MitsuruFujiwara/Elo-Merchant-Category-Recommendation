...
def additional_features(df):
    ...
    df['hist_observation_date']= df.apply(lambda x: pd.to_datetime(x['hist_purchase_date_max'])-pd.DateOffset(months=x['hist_month_lag_max']), axis=1)
    df['new_observation_date']=df.apply(lambda x: np.nan if np.isnan(x['new_month_lag_min']) else pd.to_datetime(x['new_purchase_date_min'])-pd.DateOffset(months=x['new_month_lag_min']-1), axis=1)
    ...
    df['month_diff_ratio'] = df['new_month_diff_sum']/df['hist_month_diff_sum']
    df['month_lag_mean'] = df['new_month_lag_mean']+df['hist_month_lag_mean']
    df['month_lag_max'] = df['new_month_lag_max']+df['hist_month_lag_max']
    df['month_lag_min'] = df['new_month_lag_min']+df['hist_month_lag_min']
    df['month_lag_ratio'] = df['new_month_lag_mean']/df['hist_month_lag_mean']
    ...
    df['category_1_ratio'] = df['new_category_1_mean']/df['hist_category_1_mean']
    df['category_2_ratio'] = df['new_category_2_mean']/df['hist_category_2_mean']
    df['category_3_ratio'] = df['new_category_3_mean']/df['hist_category_3_mean']
    ...
    df['price_ratio'] = df['new_price_mean'] / df['hist_price_mean']
    df['duration_mean'] = df['new_duration_mean']+df['hist_duration_mean']
    df['duration_min'] = df['new_duration_min']+df['hist_duration_min']
    df['duration_max'] = df['new_duration_max']+df['hist_duration_max']
    df['duration_ratio'] = df['new_duration_mean'] / df['hist_duration_mean']
    df['amount_month_ratio_mean']=df['new_amount_month_ratio_mean']+df['hist_amount_month_ratio_mean']
    df['amount_month_ratio_min']=df['new_amount_month_ratio_min']+df['hist_amount_month_ratio_min']
    df['amount_month_ratio_max']=df['new_amount_month_ratio_max']+df['hist_amount_month_ratio_max']
    df['amount_month_ratio_ratio']=df['new_amount_month_ratio_sum'] / df['hist_amount_month_ratio_sum']
